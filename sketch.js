...
function setup() {
  ...
  play.addImage("res3",play_img);
  play.addImage("pre3",play_press);
  play.setCollider("rectangle", 0,.5, 28,18)
  play.scale = 7;
...
function draw() {
  ...
  if(gameState == "stand-by"){
    ...
    if(menu_text==false&&mousePressedOver(play)){
      play.visible = false;
    ...
    if(menu_text==false&&touches.length>0){
      play.visible = false;
      arrow1.visible = false;
    ...
    if(mousePressedOver(exit)||touches.length>0){
      play.visible = true;
      menu.visible = false;
      exit.visible = false;
      menu_text = false;
  ...
  if(menu_text==true){
    ...
    fill("red");
    text("Cada vez que choques con un obstáculo, perderás 10 puntos, si tu puntuación",menu.x-475,menu.y-38);
    text(" llega a cero o chocas con los planeadores que te siguen, seras capturado y",menu.x-450,menu.y+16);
    text(" habrás perdido",menu.x-100, menu.y+60)
  }
...
function follow(){
  trail1 = createSprite(bike1.x,bike1.y,15,20)
  trail1.shapeColor = "cyan";
  trail1.lifetime = 300;

  trail2 = createSprite(glider1.x,glider1.y+45,15,20)
  trail2.shapeColor = "red";
  trail2.velocityY =5;
  trail2.lifetime = 300;

  trail3 = createSprite(glider2.x,glider2.y+45,15,20)
